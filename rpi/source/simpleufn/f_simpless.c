...
int hhb_modinit(void);
void hhb_modexit(void);
...
DECLARE_USB_FUNCTION(HorseSink, source_sink_alloc_inst,
		source_sink_alloc_func);

static int __init hhsslb_modinit(void)
{
	int ret;

        printk("hhsslb: loading module\n");

	ret = usb_function_register(&HorseSinkusb_func);
	if (ret)
		return ret;
	ret = hhb_modinit();
	if (ret)
		usb_function_unregister(&HorseSinkusb_func);
	return ret;
}

static void __exit hhsslb_modexit(void)
{
	usb_function_unregister(&HorseSinkusb_func);
	hhb_modexit();
}
module_init(hhsslb_modinit);
module_exit(hhsslb_modexit);
